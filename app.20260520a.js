(function () {
  ...
  function bindEvents() {
    ...
    els.controls.addEventListener("click", function (event) {
      if (event.target.dataset && event.target.dataset.gridType) {
        var previousGridType = state.gridType;
        setGridType(event.target.dataset.gridType, true);
        if (state.gridType !== previousGridType) {
          trackEvent("grid_type_change", {
    ...
    els.controls.addEventListener("change", function (event) {
      ...
      handleControlInput(event.target);
      if (event.target === els.labels) {
        trackEvent("label_toggle", {
          enabled: els.labels.checked
        });
      }
      if (event.target === els.paperFormat) {
        trackEvent("paper_format_change", {
          paper_format: els.paperFormat.value
        });
      }
    });
  ...
  function updateLabelPositionField() {
    ...
    var labelsEnabled = els.labels.checked;
    els.labelPositionField.classList.toggle("is-disabled", !labelsEnabled);
    els.labelPositionField.setAttribute("aria-disabled", labelsEnabled ? "false" : "true");

    if (els.labelPosition) {
      els.labelPosition.disabled = !labelsEnabled;
    }
  }
  ...
  function downloadPng() {
    ...
    renderForExport().toBlob(function (blob) {
      ...
      trackEvent("grid_download", {
        grid_type: state.gridType,
        rows: getNumber(els.rows, 1, MAX_GRID_COUNT, defaults.rows),
        columns: getNumber(els.cols, 1, MAX_GRID_COUNT, defaults.cols),
        labels_enabled: els.labels.checked,
        paper_format: els.paperFormat ? els.paperFormat.value : undefined,
        grayscale_enabled: state.grayscale
      });
    }, "image/png");
  }
